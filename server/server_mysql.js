...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "Load contact message error:",
        error
      );

      return res.status(500).json({
        message:
          "Cannot load contact message",
        error: error.message,
      });
    }
  }
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(
        `MEN'S Fashion backend (MySQL) running on http://localhost:${PORT}`
      );
    });
  } catch (error) {
    console.error(
    ...
    console.error(
      "Make sure MySQL is running and check db_mysql.js and .env"
    );
